...
function renderAchievements() {
  ...
  achievementData.forEach((achievement, index) => { const done = state.achievements.includes(index + 1), value = done ? achievement[3] : 0, percent = done ? 100 : 0; const card = document.createElement("div"); card.className = `achievement ${done ? "done" : ""}`; card.innerHTML = `<div class="achievement-heading"><span class="ach-icon">${achievement[0]}</span><b>${achievement[1]}</b></div><div class="achievement-content"><small>${achievement[2]}</small><div class="ach-progress"><i style="width:${percent}%"></i></div><small>${value} / ${achievement[3]}</small></div>`; element.appendChild(card); });
}
